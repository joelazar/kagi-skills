...
BIN="$BIN_DIR/kagi-search"

needs_build=0
if [[ ! -x "$BIN" ]]; then
  needs_build=1
else
  for src in "$BASE_DIR"/*.go "$BASE_DIR"/go.mod "$BASE_DIR"/go.sum; do
...
if [[ "$needs_build" -eq 1 ]]; then
  ...
  if command -v go >/dev/null 2>&1; then
    ...
    if [[ -n "$GO_VERSION" ]]; then
      ...
      if [[ "$GO_MAJOR" -lt 1 || ( "$GO_MAJOR" -eq 1 && "$GO_MINOR" -lt 26 ) ]]; then
        ...
        echo "Building kagi-search from source..." >&2
  ...
  if [[ ! -x "$BIN" ]]; then
    ...
    echo "kagi-search binary not found. Download pre-built binary from GitHub releases?" >&2
    echo "  $URL" >&2
